...
const NAV = [
  ...
  { to: "/reseller/licenses", label: "Minhas licenças", icon: KeyRound },
  { to: "/reseller/generate", label: "Gerar licença", icon: PlusCircle },
  { to: "/reseller/tokens", label: "Meus tokens", icon: Webhook },
] as const;
  ...
  return (
    <aside className="flex h-screen w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
      ...
      <nav className="flex-1 space-y-1 px-3 py-2">
        {NAV.map((item) => {
          const active =
            item.to === "/reseller" ? pathname === item.to : pathname.startsWith(item.to);
